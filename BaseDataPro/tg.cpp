...
void CTG::Draw(QPainter *painter, long nElapsed, double nDiploid, QPoint offset, int type, bool zczx)
{
    if (type == 0x55)
    {
...
void CTG::Draw_Th_kb(QPainter *painter, long nElapsed, double nDiploid, QPoint offset)
{
    ...
    if ((getIsTGANDown() == true || getCmdflag()) && ((nElapsed % 2) == 0))
    {
    ...
    if (getIsTGANFB() == true)
    {
        ...
        painter->drawLine(m_rectTGAN.topLeft(), m_rectTGAN.bottomRight()); //绘制按钮封锁
        painter->drawLine(m_rectTGAN.topRight(), m_rectTGAN.bottomLeft()); //绘制按钮封锁
    }
...
void CTG::setTG_StateOfLS(int state)
{
    if ((state & 0xC0) == 0x40)
    {
        setIsTGANDown(true);
...
void CTG::setIsTGANFB(bool flag)
{
    isTGANFB = flag;
}
bool CTG::getIsTGANFB()
{
    return isTGANFB;
}

void CTG::setCmdflag(bool flag)
{
    cmdflag = flag;
}
