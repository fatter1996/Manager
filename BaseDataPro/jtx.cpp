...
void CJTX::Draw(QPainter *painter,long nElapsed,float nDiploid,QPoint offset,int type,bool zczx)
{
    //坐标变换 2021.1.11 BJT
...
void CJTX::setVollover(QPoint pt_Base)
{
    ...
    setTextRect(QRectF(pt_Base.x() - (getTextRect().right()-pt_Base.x()),
                       pt_Base.y() - (getTextRect().bottom()-pt_Base.y()),
                       getTextRect().width(),getTextRect().height()));
    x1=getp1().x();
    x2=getp2().x();
    y1=getp1().y();
    y2=getp2().y();
