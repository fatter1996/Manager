...
int PullFlow::SDLPlayerInit(void* winid)
{
    ...
    sdlTexture = SDL_CreateTexture(sdlRenderer, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING, GlobalDataV::m_pConfigInfo->width, GlobalDataV::m_pConfigInfo->height);

    sdlRect.x = 0;
    sdlRect.y = 0;
    sdlRect.w = GlobalDataV::m_pConfigInfo->width;
    sdlRect.h = GlobalDataV::m_pConfigInfo->height;
...
int PullFlow::SDLPlayerStart()
{
    ...
    SwsContext* pImgConvertCtx = sws_getContext(pVidoeCodecCtx->width, pVidoeCodecCtx->height, pVidoeCodecCtx->pix_fmt,
            GlobalDataV::m_pConfigInfo->width, GlobalDataV::m_pConfigInfo->height, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL);
    ...
    //申请缓存
    int size = av_image_get_buffer_size(AV_PIX_FMT_YUV420P, GlobalDataV::m_pConfigInfo->width, GlobalDataV::m_pConfigInfo->height, 1);
    ...
    while (isRunning)
    {
        ...
        if (packet->stream_index == videoIndex)
        {
            ...
            while (avcodec_receive_frame(pVidoeCodecCtx, pFrame) >= 0)
            {
                ...
                pOutFrame->width = GlobalDataV::m_pConfigInfo->width;
                pOutFrame->height = GlobalDataV::m_pConfigInfo->height;
                ...
                {
                    av_fifo_generic_write(pVideoFifo, pOutFrame->data[0], pOutFrame->linesize[0] * GlobalDataV::m_pConfigInfo->height, NULL);
                    av_fifo_generic_write(pVideoFifo, pOutFrame->data[1], pOutFrame->linesize[1] * GlobalDataV::m_pConfigInfo->height / 2, NULL);
                    av_fifo_generic_write(pVideoFifo, pOutFrame->data[2], pOutFrame->linesize[2] * GlobalDataV::m_pConfigInfo->height / 2, NULL);
...
void PullFlow::VideoPlay()
{
    ...
    av_image_fill_arrays(pOutFrame->data, pOutFrame->linesize, frame_buf, AV_PIX_FMT_YUV420P, GlobalDataV::m_pConfigInfo->width, GlobalDataV::m_pConfigInfo->height, 1);
...
void PullFlow::timerEvent(QTimerEvent * ev)
{
    ...
    {
        if(lockedState != GlobalDataV::IsSessionLocked())
        {
