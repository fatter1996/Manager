...
int FileRecord::VideoCaptureInit()
{
    ...
    av_dict_set_int(&options, "framerate", GlobalDataV::m_pConfigInfo->frameRate, AV_DICT_MATCH_CASE); //帧率
    av_dict_set_int(&options, "offset_x", GlobalDataV::m_pConfigInfo->desktopRange_x, AV_DICT_MATCH_CASE); //录制区域的起始坐标-x
    av_dict_set_int(&options, "offset_y", GlobalDataV::m_pConfigInfo->desktopRange_y, AV_DICT_MATCH_CASE); //录制区域的起始坐标-y
    av_dict_set(&options, "video_size", QString("%1x%2").arg(GlobalDataV::m_pConfigInfo->desktopWidth).arg(GlobalDataV::m_pConfigInfo->desktopHeight).toLatin1(), AV_DICT_MATCH_CASE); //录制区域大小
...
int FileRecord::AudioCaptureInit()
{
    ...
    while(flag <= 3)
    {
        ...
        //}
        ret = avformat_open_input(&pAudioInFormatCtx, GlobalDataV::m_pConfigInfo->url.toLatin1(), NULL, NULL);
...
int FileRecord::H264RecodeInit()
{
    ...
    pH264CodecCtx->width = GlobalDataV::m_pConfigInfo->desktopWidth;
    pH264CodecCtx->height = GlobalDataV::m_pConfigInfo->desktopHeight;
    pH264CodecCtx->time_base.num = 1;
    pH264CodecCtx->time_base.den = GlobalDataV::m_pConfigInfo->frameRate;	//帧率（即一秒钟多少张图片）
    pH264CodecCtx->bit_rate = GlobalDataV::m_pConfigInfo->videoBitRate;	//比特率（调节这个大小可以改变编码后视频的质量）
    pH264CodecCtx->rc_max_rate = GlobalDataV::m_pConfigInfo->videoBitRate;
    pH264CodecCtx->rc_min_rate = GlobalDataV::m_pConfigInfo->videoBitRate;
...
int FileRecord::AacRecodeInit()
{
    ...
    pAacCodecCtx->sample_rate= GlobalDataV::m_pConfigInfo->sampleRate; //音频采样率
    pAacCodecCtx->channels = 2;
    pAacCodecCtx->channel_layout = av_get_default_channel_layout(pAacCodecCtx->channels);
    pAacCodecCtx->bit_rate = GlobalDataV::m_pConfigInfo->audioBitRate;
...
int FileRecord::RecordStart()
{
    ...
    int size = av_image_get_buffer_size(AV_PIX_FMT_YUV420P, GlobalDataV::m_pConfigInfo->desktopWidth, GlobalDataV::m_pConfigInfo->desktopHeight, 1);
    ...
    av_image_fill_arrays(pOutFrame->data, pOutFrame->linesize, frame_buf, AV_PIX_FMT_YUV420P,
           GlobalDataV::m_pConfigInfo->desktopWidth, GlobalDataV::m_pConfigInfo->desktopHeight, 1);
    ...
    while (isRunning)
    {
        ...
        if(currTime.toTime_t() - startTime.toTime_t() > GlobalDataV::m_pConfigInfo->maxTime)
        {
            emit RecordEnd();
            break;
        }
        ...
        if(ptsInterval <= 0 || !isAudioInit) //视频
        {
            ...
            if(avcodec_receive_packet(pH264CodecCtx, pOutPkt) >= 0)
            {
                ...
                pOutPkt->pts = VideoFrameIndex * pOutFormatCtx->streams[outVideoIndex]->time_base.den / GlobalDataV::m_pConfigInfo->frameRate;
                pOutPkt->dts = pOutPkt->pts;
                cur_pts_v = pOutPkt->pts;

                pOutPkt->duration = ((pOutFormatCtx->streams[0]->time_base.den / pOutFormatCtx->streams[videoIndex]->time_base.num) / GlobalDataV::m_pConfigInfo->videoBitRate);
