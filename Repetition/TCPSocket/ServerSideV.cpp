#include "ServerSideV.h"
#include <QDebug>
#include <QAbstractSocket>
#include "Repetition/GlobalData.h"
#pragma execution_character_set("utf-8")

ServerSideV::ServerSideV(QObject* parent)
{
    Q_UNUSED (parent)
    //log = new QFile("TcpLog.txt");
    //log->open(QIODevice::WriteOnly | QIODevice::Append);
    m_pServer = new QTcpServer;
    connect(m_pServer, &QTcpServer::newConnection, this, &ServerSideV::onConnection);
    m_pServer->listen(QHostAddress(GlobalDataV::m_pConfigInfo->localIp), GlobalDataV::m_pConfigInfo->localPORT); //绑定本地IP和端口
}

ServerSideV::~ServerSideV()
{
    if(m_pServer)
    {
        m_pServer->close();
...
void ServerSideV::Unpacking(QByteArray array, QTcpSocket* m_pSocket)
{
    if(array.length() <= 10)
        return;
    //长度校验
    int lenght = GlobalDataV::getBigEndian_2Bit(array.mid(4,2));
    ...
    else
    {
        ...
        //qDebug() << "--- Receive ---";
        //qDebug() << receiveStr.toHex();
        emit ReceiveData(array);
    }

}

QByteArray ServerSideV::HeartBeat()
{
    QByteArray heart;
...
QByteArray ServerSideV::Receipt()
{
    QByteArray heart;
    heart.append(QByteArray::fromHex("efefefef"));
...
void ServerSideV::onError(QAbstractSocket::SocketError error)
{
